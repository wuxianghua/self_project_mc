package net.imoran.auto.music.widgets.progress;
...
import net.imoran.auto.music.widgets.progress.utils.Constant;
import net.imoran.auto.music.widgets.progress.utils.MiscUtil;
...
public class CircleProgress extends View {
    ...
    //圆形轨迹上的小圆 坐标 
    private float CIRCLE_CENTER_X, CIRCLE_CENTER_Y, CIRCLE_R;
    ...
    private void initPaint() {
        ...
        mBoundPaint.setColor(Color.parseColor("#9ac3f1"));
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mCenterPoint.y = h / 2;
        //绘制圆弧的边界
        mRectF.left = mCenterPoint.x - mRadius - maxArcWidth / 2 + circleRadius / 2;
        mRectF.top = mCenterPoint.y - mRadius - maxArcWidth / 2 + circleRadius / 2;
        mRectF.right = mCenterPoint.x + mRadius + maxArcWidth / 2 - circleRadius / 2;
        mRectF.bottom = mCenterPoint.y + mRadius + maxArcWidth / 2 - circleRadius / 2;
        //计算文字绘制时的 baseline
...
//                + ";圆的外接矩形 = " + mRectF.toString());
        ...
        CIRCLE_CENTER_X = (mRectF.left + (mRectF.right - mRectF.left) / 2F);
        CIRCLE_CENTER_Y = (mRectF.top + (mRectF.bottom - mRectF.top) / 2F);
        CIRCLE_R = (mRectF.right - mRectF.left) / 2F;
    }
    ...
    private void drawArc(Canvas canvas) {
        ...
        canvas.restore();
        //绘制轨迹上的小圆
        double[] point = initPointsCircular(currentAngle);
        canvas.drawCircle((float) point[0], (float) point[1], circleRadius, mBoundPaint);
    }


    private double[] initPointsCircular(float currentAngle) {
        float angel = currentAngle - 180;
        double x = CIRCLE_CENTER_X - CIRCLE_R * Math.sin(Math.PI * angel / 180);
        double y = CIRCLE_CENTER_Y + CIRCLE_R * Math.cos(Math.PI * angel / 180);
